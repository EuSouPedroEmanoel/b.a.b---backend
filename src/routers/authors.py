...
from src.schemas import AuthorCreate, AuthorPublic, FilterPage, PaginatedResponse
...
async def get_or_create_author_by_name(session: AsyncSession, raw_name: str) -> Author:
...
async def list_authors(
    ...
    if q and q.strip():
        raw = q.strip()
        sttm = sttm.where((Author.name.ilike(f'%{raw}%')) | (Author.slug.ilike(f'%{raw}%')))
...
async def create_author(payload: AuthorCreate, session: Session, user: StaffOnly):
    name = display_name_author(payload.name)
    if not name:
        raise HTTPException(status_code=HTTPStatus.UNPROCESSABLE_ENTITY, detail='Invalid author name')
    slug = slugify_author(name)
    existing = await session.scalar(select(Author).where(Author.slug == slug))
    if existing:
        raise HTTPException(status_code=HTTPStatus.CONFLICT, detail='Author already exists')
    author = Author(name=name, slug=slug)
    session.add(author)
    try:
...
@router.delete('/{author_id}', status_code=HTTPStatus.OK)
